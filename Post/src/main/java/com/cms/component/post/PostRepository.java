...
public interface PostRepository extends JpaRepository<PostEntity, Long> {
    ...
    @Query(value = "select v.* from post v LEFT JOIN Content c ON  c.POSTID = v.POSTID where v.status = 1 and c.status = 1 AND (:keyword is null or LOWER(v.author||c.content) LIKE (LOWER('%'||:keyword||'%') collate binary_ai)) ",
            countQuery = "select count(v.*) from post v LEFT JOIN Content c ON  c.POSTID = v.POSTID where v.status = 1 and c.status = 1 AND (:keyword is null or LOWER(v.author||c.content) LIKE (LOWER('%'||:keyword||'%') collate binary_ai)) " , nativeQuery = true)
    Page<PostEntity> getPaging(Pageable pageable, String keyword);
